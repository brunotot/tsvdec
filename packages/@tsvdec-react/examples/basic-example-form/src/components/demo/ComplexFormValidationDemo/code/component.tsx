...
export default function Component() {
   ...
   */
  const [form, _setForm, data] = useForm<ModelForm>(ModelForm, {
    /**
     * "resolveDecoratorArgs" is an optional parameter which defaults to () => ({}).
     */
    ...
    <>
      ...
      <FormProvider {...providerProps}>
        <Box display="flex" flexDirection="column" gap="1rem">
          <Typography>Form changed state count: {numberOfStateChanges}</Typography>
          ...
          <Box display="flex" gap={2}>
            {SUBMIT_BUTTON_TRIGGERS_VALIDATION && (
              <Button variant="contained" onClick={onSubmit} disabled={isSubmitted && !isValid}>
                Submit
              </Button>
            )}
