...
export namespace Booleans {
   ...
   */
  export type isAnyOf<TCheck, TData extends Types.ArrayType> =
  ...
  export type isObject<T> =
    NonNullable<T> extends isFunction<T>
      ? false
   ...
   * @typeParam T - The type to check.
   */
  export type isPrimitiveArray<T> =
    Arrays.getArrayType<T> extends never ? false : isPrimitive<Arrays.getArrayType<T>>;
   ...
   * @typeParam T - The type to check.
   */
  export type isObjectArray<T> =
    Arrays.getArrayType<T> extends never ? false : isObject<Arrays.getArrayType<T>>;
